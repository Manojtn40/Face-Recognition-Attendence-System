try:
    ...
    raise RuntimeError(
        "Missing dependency: dlib. Install it into the active Python interpreter. "
        "Example (may require build tools): pip install dlib==19.17.0"
    ) from e
        ...
        if self.get_face_database():
            while stream.isOpened():
                ...
                if not flag or img_rd is None:
                    logging.error("Failed to read frame from stream; exiting loop")
                    break
                kk = cv2.waitKey(1)
